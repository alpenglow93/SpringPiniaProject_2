...
/*
	...
	----------------------
			| this (Store 전체)
	range:(state) => getters에서는 state 부분만 받아서 사용
		=> state 대신 this 사용해도 됨
...
const useFoodStore = defineStore('food_store,',{
	...
	// methods: { dataRecv() }
	actions:{
		// 값을 넘겨줄떄 사용한 responseEntity는 비동기적으로 넘어와서 async 사용해줘야함
		async foodListData(){
