 ...
 * 								  -------- <input type="text" name="userid"> 이 name 값을 넣어줘야함
 ...
 * 		사용자 인증 완료
 * 		/member/login_process
 * 		--------------------- Controller 처리가 아닌 SecurityContext가 인터셉트를 해서 처리
 * 
 */
...
public class SecurityConfig {
	...
	/*
	 * 	HTTP 요청이 있는 경우 Spring Security가 어떻게 처리할지 지시 (설정)
	 * 
	 */
	@Bean
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception 
	{
		// 공격 방어
		/*
		 * CSRF Cross site Request forgery 공격자가 인증된 브라우저에서 저장된 쿠키나 세션정보를 활용해서 => 다른 사이트
		 * 요청값 전달 : 위조 => 일반 보안 =: csrf.disable()
		 */
		http.csrf(csrf -> csrf.disable())
				// 접근 권한 설정 (URL)
				.authorizeHttpRequests(auth -> auth.requestMatchers("/", "/member/**").permitAll()	// 모두 접근 가능
						.requestMatchers("/admin/**").hasRole("ADMIN")	// ADMIN 권한이 있는 사람만 접근
						.anyRequest().permitAll())	// 지정이 안 된 URL 주소 : 모두 접근 가능
				
				// 로그인 설정 => login_ok
				.formLogin(form -> form.loginPage("/member/login")	// 로그인 화면창 설정 (설정이 없는 경우 default 로그인 화면 출력)
						.loginProcessingUrl("/member/login_process")	// 가장 중요 부분 : 로그인 처리를 담당하는 URL => 가상
																		// Security에서 인터셉트가 가능
						 ...
						 */
						.usernameParameter("userid")
						.passwordParameter("userpwd")	// 로그인 처리를 위해 id,pwd를 전송
														// id = username , pwd => password
						/*
						 * 	UsernamePasswordAuthenticationFilter
						 * 	=> String username=request.getParameter("userid")
						 * 	=> String password=request.getParameter("userpwd");
						 * 	=> 인증 객체로 전송 (Authentication)
						 * 		- username=hong
						 * 		- password=1234
						 * 	=> AuthenticationManager
				...
				.rememberMe(remember -> remember.key("my-secret-key")
						...
						.tokenValiditySeconds(60 * 60 * 24)	// 저장 기간 : 1일
						.tokenRepository(persistentTokenRepository()))	// persistent_logins 테이블에 저장
						/*
						 * 	로그인
						 * 	|
						 * 	remember-me 체크 => true
	...
	/*
	 * 권한 => permitAll => hasRole('ROLE_ADMIN')
	 * 
	 ...
	 * 
	 * @Bean public JdbcUserDetailsManager jdbcUserDetailsSevice() { return null; }
	 * // 비밀번호 암호화
	 * 
	...
	public JdbcUserDetailsManager jdbcUserDetailsService() 
	{
		...
		manager.setUsersByUsernameQuery("SELECT userid as username,userpwd as password,enable " 
	 ...
	 * 	7. JdbcUserDetailsManager	: DB에서 사용자/권한 정보를 조회해서 저장하는 역할 : SQL 사용
	 ...
	 * 	16. logout					: 로그아웃 => session 해체 / cookie 삭제
	 * 
